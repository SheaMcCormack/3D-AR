...
x_displacement = 0
y_displacement = 0
rotation_theta = np.array([0, 0, 0])  # Initial rotation angles in degree
...
def main():
    ...
    while True:
        ...
        # Rotate the object along x-axis
            ...
            rotation_theta[2] -= 5
        # elif k == -1:  # No key is pressed, stop rotation
        #     rotation_theta = [0, 0, 0]  # Reset rotation angles
...
def render(img, obj, projection, marker_size, color=False):
    ...
    transformed_vertices = rotate(vertices, theta=np.radians(rotation_theta))

    for face in obj.faces:
        face_vertices = face[0]
        points = np.array([transformed_vertices[vertex - 1] for vertex in face_vertices])
...
def rotate(vertices, theta):
    ...
    transformed_vertices = np.dot(R, np.hstack((vertices, np.ones((len(vertices), 1)))).T)
    return transformed_vertices[:3, :].T
